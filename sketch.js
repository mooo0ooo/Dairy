...
function setup() {
	...
	textFont('Kaisei Haruno Umi');
	setAttributes('antialinas', true);
	...
	for (let v of padValues) {
		...
		let x = map(v.P, 0, 1, -200, 200);
		let y = map(v.A, 0, 1, -200, 200);
		let z = map(v.D, 0, 1, -200, 200);
...
function draw() {
	...
	let minDist = 20;
	for(let p of points) {
		let projected = createVector(screenX(p.pos.x, p.pos.y, p.pos.z),screenY(p.pos.x, p.pos.y, p.pos.z));
		let d = dist(mouseX, mouseY, projected.x, projected.y);
		if(d < minDist) {
			selectedLabel = p.emo.en + " (" + p.emo.ja + ")";
		}
	}
	...
	if(selectedLabel) {
		...
		text(selectedLabel, 0, -height/2 + 40);
		pop();
	}
